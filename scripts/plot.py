...
class AxisManager:
    ...
    def ylim(self):
        ...
        #p = 0.1
        #ylim[0] = ((1-p) * ylim[0]) if (ylim[0] > 0.0) else ((1+p) * ylim[0])
        #ylim[1] = ((1+p) * ylim[1]) if (ylim[1] > 0.0) else ((1-p) * ylim[1])
...
def create_legend(output_dir, legend, colors):
    ...
    #lines = []
    #for c in colors:
    lines = [axes.plot(np.zeros([1, 1]), color=c) for c in colors]
    #lines = colors
    fig_legend.legend(labels=legend, handles=lines, loc='upper center', frameon=False, ncol=len(legend))
    #plt.show()
    fig_legend.savefig('plots/legend.png', bbox_inches='tight', pad_inches=0.1)
...
def main():
    ...
    for group, params in config.items():
        ...
        create_legend(args.output_dir, params['legend'], colors)
        print(flush=True)
